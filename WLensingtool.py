...
class light_sources:
    ...
    *cluster_image.shape == seg_map.shape
    ...
    def __init__(self, x, y, a, b, phi, cluster_image, seg_map, PSF):
        ...
        if len(PSF.shape) == 2:
            if isinstance(PSF, np.ndarray):
                    if np.allclose(np.sum(PSF), 1, 1e-1, 1e-1):
                        self.PSF = PSF
                    else:
    ...
    def deconvolver(self, ID_ind, nsteps=10000, MCMC_progress=False):
        ...
        *Below, ellipticity is (a - b)/(a + b) that is need for WL; not to be confused with eps = 1 - b/a.
        ...
        def log_likelihood(params, image, uncertainty):
            ...
            if len(np.where(model_normed > 0)) < 1:
                return -np.inf
            convolution = fftconvolve(model_normed, self.PSF, mode='same')
    ...
    def shear(self, e1, e2, x, y, divx, divy):
        ...
        *divx and divy set the grid over which the averaging and calculation of the shear takes place.
        ...
        if not e1.shape == e2.shape == x.shape == y.shape:
            raise ValueError("e1, e2, x, y must have the same shape.")
        ...
        for nx in range(divx):
            for ny in range(divy):
                ...
                if len(ind_m) > 7:  # minimum 8 sources are required in a cell to get a reliable average
                    g1[ny, nx] = np.average(e1[ind_m])
                    g1_std[ny, nx] = np.std(e1[ind_m])
                    g2[ny, nx] = np.average(e2[ind_m])
